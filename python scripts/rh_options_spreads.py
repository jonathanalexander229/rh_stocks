...
for symbol in unique_symbols:
    ...
    # Process paired spread orders
    print(f"\nPaired spread orders: {len(paired_orders)}")
    ...
    # Process unpaired opening spread orders
    print(f"\nUnpaired opening spread orders: {len(unpaired_opens)}")
    for i, (order, remaining_quantity) in enumerate(unpaired_opens):
        print(f"\nAnalyzing unpaired spread order {i+1}:")
        print(f"Open: {order['order_created_at']} - Exp: {order['expiration_date']} - {order['strike_price']} - {order['opening_strategy']} - Quantity: {remaining_quantity}")
    ...
    # Calculate total opened and closed quantities for spread orders
    total_opened_spreads = sum(order['processed_quantity'] for order, _, _ in paired_orders) + sum(remaining_quantity for _, remaining_quantity in unpaired_opens)
    total_closed_spreads = sum(sum(close_order[1] for close_order in close_orders) for _, close_orders, _ in paired_orders)

    print(f"\nTotal opened spread quantity: {total_opened_spreads}")
    print(f"Total closed spread quantity: {total_closed_spreads}")
    print(f"Remaining open spread quantity: {total_opened_spreads - total_closed_spreads}")